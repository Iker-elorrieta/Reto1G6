...
public class ControladorHistorico {
    ...
    private final DateTimeFormatter fmtFecha = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm", Locale.getDefault()).withZone(ZoneId.systemDefault());
    ...
    private void inicializar() {
        this.vista.getBtnVolver().addActionListener(e -> {
            try { this.vista.setVisible(false); this.vista.dispose(); } catch (Exception ex) {}
        });
        cargarHistorico();
    }
    ...
    private void cargarHistorico() {
        try {
            ...
            if (co != null) {
                try {
                    ApiFuture<QuerySnapshot> q = co.collection("usuarios").document(usuario.getEmail()).collection("historico_workouts").get();
            ...
            for (Map<String,String> m : listaRaw) {
                ...
                if (uid == null) uid = "";
                if (uid.equalsIgnoreCase(usuario.getEmail())) listaFiltrada.add(m);
            }
            ...
            if (listaWorkouts != null) {
                for (Workout w : listaWorkouts) {
                    if (w != null && w.getNombre() != null) mapaWork.put(w.getNombre(), w);
                }
            }
            ...
            for (Map<String,String> m : listaFiltrada) {
                try {
                    ...
                    Instant inst = null;
                    if (!fechaStr.isEmpty()) {
                        try { inst = Instant.parse(fechaStr); } catch (Exception e) {
                            try { long millis = Long.parseLong(fechaStr); inst = Instant.ofEpochMilli(millis); } catch (Exception ex) { inst = null; }
                        }
                    }
                    String idw = m.getOrDefault("id_workout", "");
                    int ejercs = 0;
                    try { ejercs = Integer.parseInt(m.getOrDefault("ejercicios_hechos", "0")); } catch (Exception ex) { ejercs = 0; }
                    int tiempoSegs = 0;
                    try { tiempoSegs = Integer.parseInt(m.getOrDefault("tiempo", "0")); } catch (Exception ex) { tiempoSegs = 0; }

                    double durMin = 0.0;
                    if (mapaWork.containsKey(idw)) durMin = mapaWork.get(idw).getDuracionMinutos();

                    int porcentaje = 0;
                    if (durMin > 0.0) porcentaje = (int) Math.round((tiempoSegs / (durMin * 60.0)) * 100.0);
            ...
            items.sort(Comparator.comparing(ItemHistorico::getFechaInstant, Comparator.nullsLast(Comparator.reverseOrder())));
            ...
            for (ItemHistorico it : items) {
                ...
                Object[] row = new Object[] { fechaForm, tiempoForm, nombreWo, it.getEjercicios(), it.getPorcentaje() + "%" };
                model.addRow(row);
            }

            if (model.getRowCount() == 0) JOptionPane.showMessageDialog(null, "No hay histórico para este usuario", "Histórico vacío", JOptionPane.INFORMATION_MESSAGE);

        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Error cargando histórico: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private String formatTiempo(int segundos) {
        ...
        if (horas > 0) return String.format("%02d:%02d:%02d", horas, minutos, secs);
    ...
    private static class ItemHistorico {
        ...
        public Instant getFechaInstant() { return fechaInstant; }
        public int getTiempoSegs() { return tiempoSegs; }
        public String getIdWorkout() { return idWorkout; }
        public int getEjercicios() { return ejercicios; }
