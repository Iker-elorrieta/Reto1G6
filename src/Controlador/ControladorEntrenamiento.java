...
public class ControladorEntrenamiento {
		...
		this.vista.getBtnTerminar().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						try {
							vista.getBtnTerminar().setEnabled(false);
						} catch (Exception ex) {
						}
					}
				});
	...
	// Al pulsar Terminar mostrar mensaje inmediatamente
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				JOptionPane.showMessageDialog(null,
						"Felicidades, haz completado el " + porcentajeEnteroLocal + "% del workout",
						"Entrenamiento terminado", JOptionPane.INFORMATION_MESSAGE);
			}
		});
			...
			public void run() {
				...
				if (porcentajeEnteroOficial != porcentajeMostrado) {
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							JOptionPane.showMessageDialog(null,
									"Actualización: tu porcentaje real completado es " + porcentajeEnteroOficial + "% ",
									"Porcentaje actualizado", JOptionPane.INFORMATION_MESSAGE);
						}
					});
				}
			}
			...
			for (Ejercicio ej : ejercicios) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						javax.swing.ImageIcon icon = crearIcono(ej.getImagen(), 174, 117);
						vista.setImagenEjercicio(icon);
				...
				if (listaSeries != null) {
					for (Serie ss : listaSeries) {
						...
						if (cant > 0) {
							...
							if (cant > 1)
								tiempoTotalEjercicioSegs += (cant - 1) * tdesc;
						}
					}
				}
				...
				if (listaSeries != null) {
					for (Serie s : listaSeries) {
						...
						for (int rep = 0; rep < cantidad; rep++) {
							...
							if (cronoPorEjercicio != null && !cronoPorEjercicio.estaEnEjecucion()
									&& cronoWorkout.estaEnEjecucion()) {
								cronoPorEjercicio.iniciar();
							}
							...
							CronometroThread cronoSerie = new CronometroThread(vista.getLblTSerie(),
									tiempoSerieSegs * 1000L);
							...
							if (tiempoDescSegs > 0 && rep < (cantidad - 1)) {
								...
								CronometroThread cronoDesc = new CronometroThread(vista.getLblTDescanso(),
										tiempoDescSegs * 1000L);
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						try {
							...
							if (vistaWorkoutsPrev != null)
								vistaWorkoutsPrev.setVisible(true);
							if (vistaEjerciciosPrev != null) {
								vistaEjerciciosPrev.setVisible(false);
								vistaEjerciciosPrev.dispose();
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	private synchronized void guardarHistorico(int ejerciciosHechos, long tiempoMillis) {
		...
		if (usuario == null)
			return;
		try {
			System.out.println("Guardar historico: ejercicios=" + ejerciciosHechos + " tiempoSegs="
					+ Math.round(tiempoMillis / 1000.0));
			Firestore co = Conexion.conectar();
			if (co == null)
				return;
	...
	// Helper para cargar y escalar un ImageIcon desde varias posibles rutas (Hecho
	// con IA)
	private ImageIcon crearIcono(String ruta, int ancho, int alto) {
		...
		if (img == null) {
			try {
				java.net.URL res = ControladorEntrenamiento.class.getResource("/" + ruta);
				if (res != null)
		...
		if (ow > 0 && oh > 0 && ancho > 0 && alto > 0) {
			double scale = Math.min((double) ancho / ow, (double) alto / oh);
