	...
	private void inicializarControladorWorkouts() {
		...
		try {
			...
			for (Workout w : lista) {
				...
				if (permitido) {
					Object[] row = new Object[] { w.getNombre(), w.getNivel(),
							String.format("%.1f", w.getDuracionMinutos()), w.getVideo() };
					model.addRow(row);
				}
			}
			if (model.getRowCount() == 0) {
				javax.swing.JOptionPane.showMessageDialog(null, "No hay workouts disponibles para tu nivel", null,
	...
	private void mLoginUsuario() {
		...
		if (usuario.mAutenticarUsuario(emailId, pass)) {
			...
			try {
				...
				System.err.println("Error generando backups: ");
				ex.printStackTrace();
			}
	...
	private void mAbrirEjercicios(String workoutId) {
		try {
			...
			for (Ejercicio e : lista) {
				Object[] row = new Object[] { e.getNombre(), String.format("%.1f", e.getDuracionMinutos()),
						e.getSeriesCount(), String.format("%.1f", e.getAvgTiempoDescanso()),
						String.format("%.1f", e.getAvgTiempoSerie()) };
				model.addRow(row);
			}

			vistaEjercicios.getTableEjercicios().getSelectionModel()
					.addListSelectionListener(new ListSelectionListener() {
						...
						public void valueChanged(ListSelectionEvent e) {
							...
							final String ejercicioId = String
									.valueOf(vistaEjercicios.getTableEjercicios().getValueAt(row, 0));
								...
								if (img != null && !img.isEmpty()) {
									...
									if (orig != null) {
										...
										if (ow > 0 && oh > 0) {
											...
											java.awt.Image scaled = orig.getScaledInstance(nw, nh,
			...
			vistaEjercicios.getBtnEmpezarWO().addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						new Controlador.ControladorEntrenamiento(ventanaEntreno, lista, workoutId, usuarioActual,
								vistaWorkouts, vistaEjercicios);
						ventanaEntreno.setVisible(true);
						if (vistaWorkouts != null)
							vistaWorkouts.setVisible(false);
						if (vistaEjercicios != null) {
							vistaEjercicios.setVisible(false);
							vistaEjercicios.dispose();
						}
					} catch (Exception ex) {
						ex.printStackTrace();
						javax.swing.JOptionPane.showMessageDialog(null, "Error al iniciar entrenamiento", "Error",
