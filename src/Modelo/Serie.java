	...
	public ArrayList<Serie> mObtenerSeries(String workoutId, String ejercicioId) {
		...
		try {
			...
			ApiFuture<QuerySnapshot> query = co.collection("workouts").document(workoutId).collection("ejercicios")
					.document(ejercicioId).collection("Series").get();
			...
			for (QueryDocumentSnapshot doc : querySnapshot.getDocuments()) {
				...
				if (cantidadL != null)
					s.setCantidad(cantidadL.intValue());
				if (tiempoSerieL != null)
					s.setTiempo_serie(tiempoSerieL.intValue());
				if (tiempoDescL != null)
					s.setTiempo_descanso(tiempoDescL.intValue());
				lista.add(s);
			}
			co.close();
		} catch (Exception e) {
			...
			try {
				...
				for (WorkoutCompleto wc : backups) {
					...
					for (EjercicioConSeries ecs : wc.getEjercicios()) {
						if (!ecs.getNombre().equals(ejercicioId))
							continue;
